...
public class PitTest {
    ...
    void setUp() {
        ...
        board.getSpace(0,1).setBoardElement(new Pit());
    }

    @Test
    void damagedWhenLandedOn(){
        Board board = gameController.board;
        board.setPhase(Phase.ACTIVATION);
        Player current = board.getCurrentPlayer();
        current.getProgramField(0).setCard(new CommandCard(Command.FORWARD));
        gameController.executePrograms();
        Assertions.assertEquals(PlayerState.DAMAGED, current.getState(), "player should be damaged");
    }
}
